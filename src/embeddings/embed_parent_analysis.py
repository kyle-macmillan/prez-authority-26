"""Generate dual-role Qwen embeddings for presidential-directive parent retrieval."""
...
def merge_artifact_provenance(path: Path, artifacts: list[dict]) -> list[dict]:
    """Retain valid entries for caches not regenerated by a partial run."""
    existing: list[dict] = []
    if path.is_file():
        existing = json.loads(path.read_text(encoding="utf-8")).get("artifacts", [])
    replacements = {str(artifact["output"]) for artifact in artifacts}
    return [
        artifact
        for artifact in existing
        if str(artifact.get("output", "")) not in replacements
    ] + artifacts
...
def main() -> None:
    ...
    if args.artifact in ("all", "documents"):
        artifacts.append(
            encode_artifact(
            model,
            args.input_dir / "directive_similarity_documents.jsonl",
            args.output_dir / "directive_document_embeddings.npz",
            "document_id",
            "cleaned_masked_text",
            FULL_DOCUMENT_INSTRUCTION,
    ...
    if args.artifact in ("all", "segments"):
        artifacts.append(
            encode_artifact(
            model,
            args.input_dir / "directive_operative_segments.jsonl",
            args.output_dir / "directive_operative_segment_embeddings.npz",
            "segment_id",
            "text",
            OPERATIVE_SEGMENT_INSTRUCTION,
    ...
    artifacts = merge_artifact_provenance(provenance_path, artifacts)
    provenance = {
