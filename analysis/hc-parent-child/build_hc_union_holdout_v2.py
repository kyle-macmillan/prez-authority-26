#!/usr/bin/env python3
"""Build the final 10-case disjoint HC-union confirmatory holdout."""
...
OUTPUT = DEFAULT_OUTPUT / "hc_union_holdout_v2"
...
def main() -> None:
    evidence = {
        ...
        "ieepa_holdout_v3": (
            DEFAULT_OUTPUT / "ieepa_holdout_v3" / "family_review_key.csv", ("document_id",)
        ),
        "hc_union_development": (
            DEFAULT_OUTPUT / "hc_union_holdout" / "family_review_key.csv", ("document_id",)
        ),
        ...
        "noncontrol_scope_screen": (
            DEFAULT_OUTPUT / "parent_pair_noncontrol_scope_screen.csv", ("child_id", "parent_id")
        ),
    }
    excluded_by_source = {
    ...
    proposed = balanced_sample(
        proposed_pool, 5, ("president", "document_type"), "hc_union_holdout_v2:proposed"
    )
    boundary = balanced_sample(
        boundary_pool, 5, ("president", "document_type"), "hc_union_holdout_v2:boundary"
    )
    if len(proposed) != 5 or len(boundary) != 5:
        raise ValueError("could not fill the 5 proposed / 5 boundary design")

    queue, key = [], []
    for number, (queue_type, row) in enumerate(
        [("proposed", row) for row in proposed] + [("boundary", row) for row in boundary], 1
    ):
        review_id = f"V{number:02d}"
        queue.append({
            "review_id": review_id,
            "family_id": "hc_union",
    ...
    queue_path = OUTPUT / "family_review_queue.csv"
    key_path = OUTPUT / "family_review_key.csv"
    write_csv(queue_path, queue)
    write_csv(key_path, key)
    manifest = {
        "kind": "final_hc_union_confirmatory_holdout",
    ...
    print(json.dumps({"rows": 10, "output": str(OUTPUT)}, sort_keys=True))
