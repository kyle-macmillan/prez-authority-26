...
def test_analysis_prefilter_keeps_reviewed_ocr_vesting_clause():
    rows = [{
        ...
        "doc_text": (
            "By the authority vested in my by the Constitution and the laws of the United "
            "States of America, including section 301 of title 3 of the United States Code, "
            "I hereby delegate the function."
        ),
        "doc_type": "memorandum",
        "term": "1",
    }]
...
def test_constitutional_provisions_collapse_into_specific_constitution():
    assert category(
...
def test_all_specificity_combinations_are_mutually_exclusive():
    cases = [
        ("generic_constitution_only", "By the authority vested in me by the Constitution,"),
        ("specific_constitution_only", "By the authority vested in me by Article II of the Constitution,"),
        ("specific_constitution_only", "By the authority vested in me as Commander in Chief,"),
        ("generic_statute_only", "By the authority vested in me by the laws of the United States,"),
        ("specific_statute_only", "By the authority vested in me by the Example Act,"),
        ("specific_statute_only", "By the authority vested in me by section 301 of title 3, United States Code,"),
        (
            "generic_constitution_and_generic_statute",
        ...
        (
            "generic_constitution_and_specific_statute",
            "By the authority vested in me by the Constitution and section 301 of title 3, United States Code,",
        ),
        (
            "specific_constitution_and_generic_statute",
            "By the authority vested in me by Article II of the Constitution and under the laws of the United States,",
        ),
        (
        ...
        (
            "specific_constitution_and_specific_statute",
            "By the authority vested in me by Article II of the Constitution and section 301 of title 3, United States Code,",
        ),
        (
            "specific_constitution_and_generic_statute",
            "By the authority vested in me as Commander in Chief and under the laws of the United States,",
        ),
        (
            "specific_constitution_and_specific_statute",
        ...
        (
            "specific_constitution_and_specific_statute",
            "By the authority vested in me as Commander in Chief and by section 301 of title 3, United States Code,",
        ),
    ]
    assert {expected for expected, _ in cases} == set(CATEGORIES) - {
        "no_vesting_clause", "other_vesting_authority"
    }
    for expected, clause in cases:
...
def test_act_and_statutory_section_collapse_into_specific_statute():
    clause = (
...
def test_authorizing_proclamation_recital_ignores_constitutional_history():
    ...
    assert category(clause) == "specific_statute_only"
    assert category(
...
def test_html_contains_every_category_and_count_column():
    ...
    assert f"{len(CATEGORIES)} presidential directives" in report
    assert len(CATEGORIES) == 10
    assert len(SUBSTANTIVE_CATEGORIES) == 8
    assert "No clause</abbr>" in report
    assert "Other</abbr>" in report
    ...
    assert 'data-display="number" aria-pressed="true">Number</button>' in report
    assert 'data-display="proportion" aria-pressed="false">Proportion</button>' in report
    assert 'main[data-display="proportion"] .category-count .raw-count' in report
